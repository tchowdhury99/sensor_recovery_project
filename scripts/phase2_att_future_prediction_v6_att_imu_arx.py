...
MODEL_DIR = os.path.join(BASE, "models_v6_att_imu_arx")
RESULT_DIR = os.path.join(BASE, "prediction_results_v6_att_imu_arx")
FIG_DIR = os.path.join(BASE, "figures", "future_prediction_v6_att_imu_arx")
...
def find_matching_file(base_prefix, suffix):
    """
    Example:
    baseline_01_hover_ATT.csv -> baseline_01_hover_IMU.csv
    """
    candidate = os.path.join(CSV_DIR, base_prefix + suffix)
...
def merge_att_imu(att_path, imu_path):
    ...
    needed_att = ["t"] + ATT_COLS + [c for c in DES_COLS if c in att.columns]
    needed_imu = ["t"] + [c for c in GYRO_COLS if c in imu.columns]
    ...
    att = att[needed_att].apply(pd.to_numeric, errors="coerce")
    imu = imu[needed_imu].apply(pd.to_numeric, errors="coerce")
...
def build_dataset(df, feature_cols, target_col):
    X_rows = []
    y_delta_rows = []
    y_now_rows = []
    y_future_rows = []
    ...
    for col in feature_cols:
        feature_values[col] = df[col].to_numpy(dtype=float)
    ...
    for k in range(start_k, len(df) - 1):
        ...
        for lag in range(INPUT_LAGS):
            idx = k - lag
            for col in feature_cols:
                row.append(feature_values[col][idx])
...
def plot_prediction(t, y_true, y_pred, y_naive, y_cv, title, out_path):
    ...
    plt.plot(t, y_pred, label="Trained ATT+IMU ARX prediction", linewidth=1.5)
...
def plot_error(t, y_true, y_pred, y_naive, y_cv, title, out_path):
    ...
    plt.plot(t, y_pred - y_true, label="Trained ATT+IMU ARX error", linewidth=1.5)
...
def process_pair(att_path, imu_path):
    ...
    des_cols = [c for c in DES_COLS if c in df.columns]
    feature_cols = des_cols + GYRO_COLS
    ...
    for target in TARGETS:
        ...
        data = build_dataset(df, feature_cols, target)
        ...
        print(f"ATT+IMU ARX RMSE: {model_rmse:.8f}")
        ...
        rows.append({
            ...
            "trained_ATT_IMU_ARX_MAE": model_mae,
            "trained_ATT_IMU_ARX_RMSE": model_rmse,
            "trained_ATT_IMU_ARX_R2": model_r2,
            "naive_MAE": naive_mae,
        ...
        pred_out[f"{target}_trained_ATT_IMU_ARX_prediction"] = y_pred
        pred_out[f"{target}_naive_prediction"] = y_naive_test
        pred_out[f"{target}_constant_velocity_prediction"] = y_cv_test
        pred_out[f"{target}_trained_ATT_IMU_ARX_error"] = y_pred - y_true_test

        model_obj = {
            "att_file": att_name,
            "imu_file": imu_name,
            "target": target,
            "model_type": "trained_att_imu_delta_arx_one_step_future_predictor",
        ...
        model_path = os.path.join(MODEL_DIR, f"{base}_{target}_ATT_IMU_ARX_model.pkl")
        with open(model_path, "wb") as f:
            pickle.dump(model_obj, f)

        fig_path = os.path.join(FIG_DIR, f"{base}_{target}_ATT_IMU_ARX_prediction.png")
        plot_prediction(t_test, y_true_test, y_pred, y_naive_test, y_cv_test,
                        f"{att_name}: ATT+IMU trained ARX {target}[k+1]", fig_path)

        err_path = os.path.join(FIG_DIR, f"{base}_{target}_ATT_IMU_ARX_error.png")
        plot_error(t_test, y_true_test, y_pred, y_naive_test, y_cv_test,
                   f"{att_name}: ATT+IMU trained ARX {target}[k+1] error", err_path)

    pred_path = os.path.join(RESULT_DIR, f"{base}_ATT_IMU_ARX_prediction_outputs.csv")
...
def main():
    print("\n========== PHASE 2 V6: TRAINED ATT + IMU ARX FUTURE PREDICTION ==========")
    ...
    for att_file in att_files:
        ...
        imu_path = find_matching_file(prefix, "_IMU.csv")
    ...
    if not all_rows:
        raise RuntimeError("No ATT+IMU prediction results were produced.")

    summary = pd.DataFrame(all_rows)
    summary_path = os.path.join(RESULT_DIR, "phase2_v6_att_imu_trained_ARX_summary.csv")
    summary.to_csv(summary_path, index=False)

    print("\n========== V6 ATT+IMU TRAINED ARX SUMMARY ==========")

    cols = [
        "att_file",
        "target",
        "best_ridge_lambda",
        "trained_ATT_IMU_ARX_RMSE",
        "naive_RMSE",
        "constant_velocity_RMSE",
        "rmse_improvement_percent_over_naive",
        "rmse_improvement_percent_over_cv",
        "trained_ATT_IMU_ARX_R2"
    ]
