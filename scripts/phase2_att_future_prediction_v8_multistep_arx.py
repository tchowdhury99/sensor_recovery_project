...
MODEL_DIR = os.path.join(BASE, "models_v8_multistep_arx")
RESULT_DIR = os.path.join(BASE, "prediction_results_v8_multistep_arx")
FIG_DIR = os.path.join(BASE, "figures", "future_prediction_v8_multistep_arx")
...
# Main correction:
# Predict 10 samples into the future, not only 1 sample.
HORIZON = 10
...
def merge_att_imu(att_path, imu_path):
    ...
    # Gyro integration features. If gyro is rad/s, convert to deg/sample.
...
def build_dataset(df, target):
    ...
    for k in range(start_k, len(df) - HORIZON):
        row = []
        ...
        y_prev = y_values[k - 1]
        y_now = y_values[k]
        y_future = y_values[k + HORIZON]

        if target == "Yaw":
            true_delta_h = angle_diff_deg(y_future, y_now)
            prev_delta = angle_diff_deg(y_now, y_prev)
        else:
            true_delta_h = y_future - y_now
            prev_delta = y_now - y_prev

        # Baseline 1: hold current state until future horizon.
        y_naive = y_now

        # Baseline 2: constant velocity extrapolated HORIZON samples.
        y_cv = y_now + HORIZON * prev_delta

        X_rows.append(row)
        y_delta_h_rows.append(true_delta_h)
        y_now_rows.append(y_now)
        y_future_rows.append(y_future)
        y_naive_rows.append(y_naive)
        y_cv_rows.append(y_cv)
        t_rows.append(t_values[k + HORIZON])
    ...
    return {
        "X": np.asarray(X_rows),
        "y_delta_h": np.asarray(y_delta_h_rows),
...
def choose_lambda(X_train, y_train):
    ...
    for lam in RIDGE_LAMBDAS:
        ...
        pred_delta = predict_ridge(X_val_s, beta)
        _, rmse, _ = compute_metrics(y_val, pred_delta)
...
def plot_prediction(t, y_true, y_pred, y_naive, y_cv, title, out_path):
    ...
    plt.plot(t, y_true, label=f"True future value k+{HORIZON}", linewidth=2)
    plt.plot(t, y_pred, label=f"Trained ARX prediction k+{HORIZON}", linewidth=1.5)
    plt.plot(t, y_naive, label="Naive hold baseline", linestyle="--", linewidth=1.0)
...
def process_pair(att_path, imu_path):
    ...
    print(f"Prediction horizon: {HORIZON} samples ahead")

    rows = []
    pred_out = pd.DataFrame()

    for target in TARGETS:
        print(f"\n--- Target: {target}[k+{HORIZON}] ---")

        data = build_dataset(df, target)

        X = data["X"]
        y_delta_h = data["y_delta_h"]

        split = int(TRAIN_RATIO * len(X))

        X_train = X[:split]
        y_train = y_delta_h[:split]
        ...
        print(f"Multi-step ARX RMSE: {model_rmse:.8f}")
        ...
        rows.append({
            ...
            "trained_multistep_ARX_MAE": model_mae,
            "trained_multistep_ARX_RMSE": model_rmse,
            "trained_multistep_ARX_R2": model_r2,
            "naive_MAE": naive_mae,
        ...
        pred_out[f"{target}_true_future_kplus{HORIZON}"] = y_true_test
        pred_out[f"{target}_trained_multistep_ARX_prediction"] = y_pred
        pred_out[f"{target}_naive_prediction"] = y_naive_test
        pred_out[f"{target}_constant_velocity_prediction"] = y_cv_test
        pred_out[f"{target}_trained_multistep_ARX_error"] = y_pred - y_true_test
        ...
        model_obj = {
            ...
            "imu_file": imu_name,
            "target": target,
            "model_type": "trained_multistep_physics_informed_arx",
            "formula": f"x_hat[k+{HORIZON}] = x[k] + W * phi[k]",
            "horizon_samples": HORIZON,
        ...
        model_path = os.path.join(
            MODEL_DIR,
            f"{base}_{target}_multistep_ARX_h{HORIZON}_model.pkl"
        )
        ...
        fig_path = os.path.join(
            FIG_DIR,
            f"{base}_{target}_multistep_ARX_h{HORIZON}_prediction.png"
        )
        ...
        plot_prediction(
            ...
            y_naive_test,
            y_cv_test,
            f"{att_name}: trained ARX {target}[k+{HORIZON}] prediction",
            fig_path
        )

        err_path = os.path.join(
            FIG_DIR,
            f"{base}_{target}_multistep_ARX_h{HORIZON}_error.png"
        )
        ...
        plot_error(
            ...
            y_naive_test,
            y_cv_test,
            f"{att_name}: trained ARX {target}[k+{HORIZON}] error",
            err_path
        )

    pred_path = os.path.join(
        RESULT_DIR,
        f"{base}_multistep_ARX_h{HORIZON}_prediction_outputs.csv"
    )
...
def main():
    print("\n========== PHASE 2 V8: MULTI-STEP TRAINED ARX FUTURE PREDICTION ==========")
    print("Training-based system-identification model.")
    print(f"Prediction horizon: k+{HORIZON} samples.")
    ...
    if not all_rows:
        raise RuntimeError("No V8 results produced.")

    summary = pd.DataFrame(all_rows)
    summary_path = os.path.join(
        RESULT_DIR,
        f"phase2_v8_multistep_ARX_h{HORIZON}_summary.csv"
    )

    summary.to_csv(summary_path, index=False)

    print("\n========== V8 MULTI-STEP ARX SUMMARY ==========")
